...
class HOG:
	def __init__(self, vector, idImage):
		...
		self.__idImg = idImage # private attribute
	def getId(self):
		return self.__idImg
	def getVector(self):
		return self.__vector
	def setIdImg(self, idImg):
		self.__idImg = idImg
...
def computeDistanceEuclide(vectorA, vectorB, count, idImg):
	dis = 0
...
def findMinDistance(vector, lstVector, count):
    vt = 0
    min, count = computeDistanceEuclide(vector, lstVector[0].getVector(), count, '1')
    for i in range(1, len(lstVector)):
        if min > computeDistanceEuclide(vector, lstVector[i].getVector(), count, '1')[0]:
            min, count = computeDistanceEuclide(vector, lstVector[i].getVector(), count, '1')
            vt = i
    return [vt, count]
	...
	if rects != []:
		...
		for rect in rects:
			...
			for vec in arrVector:
				dis, count = computeDistanceEuclide(vec, vector, count, '1')
				arrDis.append(dis)
	...
	if rects != []:
		...
		for rect in rects:
			...
			for i in range(0, len(lstVector)):
				arrDis.append(computeDistanceEuclide(vector, lstVector[i].getVector(), count, '1'))
			...
			while True:
				...
				if model.__contains__('Node'):
					...
					for i in range(0, len(lstV)):
						arrDis.append(computeDistanceEuclide(vector, lstV[i].getVector(), count, '1'))
			...
			for i in range(0, len(lstImg)):
				dis, count = computeDistanceEuclide(lstImg[i].getVector(), vector, count, '1')
			...
			lstId = []
			for hog in lstImg:
				if checkId(lstId, hog.getId()) == True:
					continue
				if '_' in hog.getId():
					hog.setIdImg(hog.getId().split('_')[0])
				count += 1
				lstImage.append(hog.getId())
				lstId.append(hog.getId())
				dem += 1
				if dem == 100:
					break
	return [lstImage, count]

def checkId(lstId, idImg):
	if '_' in idImg:
		idImg = idImg.split('_')[0]
